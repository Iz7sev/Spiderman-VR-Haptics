...
class UDPServerProtocol(asyncio.DatagramProtocol):  #GUIから送られてきたUDPメッセージを処理するクラス
    def __init__(self, message_formatter):
        ...
        self.received_data = [] #外部から送られてきた生データを保存するリスト
    ...
    def datagram_received(self, data, addr):    #UDPメッセージごとに処理を分岐させる
        try:
            message = data.decode('utf-8', errors='replace').strip()
            msg = self.parse_message(message)

            if msg == 'set':    #'set'メッセージを受け取ったら、データの読み取りを開始する
